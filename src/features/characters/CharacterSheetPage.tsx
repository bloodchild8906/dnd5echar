...
export const CharacterSheetPage = () => {
  ...
  if (!character) {
    return (
      <EmptyState
        ...
        description="Open a character from the dashboard to view the live sheet."
      />
    );
  }
  ...
  const activeForm = character.wildShapes.activeForm
    ? character.wildShapes.forms.find((form) => form.id === character.wildShapes.activeForm?.formId)
    : null;
    ...
    <div className="page-stack print-surface">
      <section className="page-header page-header--hero">
        <div>
          ...
          <p>
            {character.raceName} {character.subraceName ? `(${character.subraceName}) ` : ''}Level{' '}
            {character.level} {character.className}
        ...
        <div className="inline-badges">
          ...
          <Badge tone="success">
            HP {character.combat.hitPoints.current}/{character.combat.hitPoints.max}
          </Badge>
      ...
      <SectionCard title="Core Stats" subtitle="Fast-adjust play view for table use.">
        ...
        <div className="stats-row stats-row--dense">
          <NumberAdjuster
            ...
            onChange={(value) =>
              patch((entry) => ({
                ...
                combat: {
                  ...entry.combat,
                  hitPoints: { ...entry.combat.hitPoints, current: value },
                },
              }))
            }
          />
          <NumberAdjuster
            ...
            onChange={(value) =>
              patch((entry) => ({
                ...entry,
                combat: { ...entry.combat, hitPoints: { ...entry.combat.hitPoints, temp: value } },
              }))
            }
          />
          <NumberAdjuster
            value={character.combat.deathSaves.successes}
            label="Death Successes"
            min={0}
            onChange={(value) =>
              patch((entry) => ({
                ...entry,
                combat: {
                  ...entry.combat,
                  deathSaves: { ...entry.combat.deathSaves, successes: Math.min(3, value) },
                },
              }))
            }
          />
          <NumberAdjuster
            ...
            onChange={(value) =>
              patch((entry) => ({
                ...
                combat: {
                  ...entry.combat,
                  deathSaves: { ...entry.combat.deathSaves, failures: Math.min(3, value) },
                },
              }))
            }
          />
        ...
        <div className="stats-row stats-row--dense">
          ...
          <div className="sheet-chip">
            Carry: {getTotalCarriedWeight(character.inventory.items).toFixed(1)} /{' '}
            {getCarryCapacity(character)} lb
          </div>
          <div className="sheet-chip">
            Attuned: {getAttunedItemCount(character.inventory.items)}
          </div>
          <div className="sheet-chip">
            Currency: {totalCurrencyInGold(character.currency).toFixed(2)} gp
          </div>
        </div>
      </SectionCard>

      <SectionCard
        title="Passives and Overrides"
        subtitle="Derived values can be overridden without losing their auto-calculated source."
      >
        <div className="form-grid form-grid--three">
          <label>
            ...
            <input
              ...
              type="number"
              value={
                character.combat.passiveOverrides.perception?.value ??
                getPassiveScore(character, 'perception')
              }
              onChange={(event) =>
                patch((entry) => ({
                  ...entry,
                  combat: {
                    ...entry.combat,
                    passiveOverrides: {
                      ...entry.combat.passiveOverrides,
                      perception: setOverride(event.target.value),
                    },
                  },
                }))
              }
          ...
          <label>
            ...
            <input
              ...
              type="number"
              value={
                character.combat.passiveOverrides.investigation?.value ??
                getPassiveScore(character, 'investigation')
              }
              onChange={(event) =>
                patch((entry) => ({
                  ...entry,
                  combat: {
                    ...
                    passiveOverrides: {
                      ...entry.combat.passiveOverrides,
                      investigation: setOverride(event.target.value),
                    },
                  },
                }))
              }
          ...
          <label>
            ...
            <input
              ...
              type="number"
              value={
                character.combat.passiveOverrides.insight?.value ??
                getPassiveScore(character, 'insight')
              }
              onChange={(event) =>
                patch((entry) => ({
                  ...entry,
                  combat: {
                    ...
                    passiveOverrides: {
                      ...entry.combat.passiveOverrides,
                      insight: setOverride(event.target.value),
                    },
                  },
                }))
              }
        ...
        <div className="inline-badges">
          {character.combat.armorClassOverride ? (
            <Badge tone="warning">
              AC override: {character.combat.armorClassOverride.reason || 'manual'}
            </Badge>
          ) : null}
          {character.combat.initiativeOverride ? (
            <Badge tone="warning">
              Initiative override: {character.combat.initiativeOverride.reason || 'manual'}
            </Badge>
          ) : null}
          {character.proficiencyBonusOverride ? (
            <Badge tone="warning">
              Proficiency override: {character.proficiencyBonusOverride.reason || 'manual'}
            </Badge>
          ) : null}
        </div>
      </SectionCard>
      ...
      {activeForm && formCompare ? (
        <SectionCard
          title="Active Form"
          subtitle="Shows what changes during a transformation and what carries over."
        >
          <div className="comparison-grid">
            <div>
              <h3>{activeForm.name}</h3>
              <p>
                {activeForm.size} {activeForm.creatureType} - CR {activeForm.challengeRating}
              </p>
              ...
              <div className="button-row">
                <button
                  ...
                  onClick={() =>
                    patch((entry) => ({
                      ...entry,
                      wildShapes: entry.wildShapes.activeForm
                        ? {
                            ...entry.wildShapes,
                            activeForm: {
                              ...entry.wildShapes.activeForm,
                              currentHp: Math.max(0, entry.wildShapes.activeForm.currentHp - 1),
                            },
                          }
                        : entry.wildShapes,
                    }))
                    ...
                    patch((entry) => ({
                      ...
                      wildShapes: entry.wildShapes.activeForm
                        ? {
                            ...
                            activeForm: {
                              ...entry.wildShapes.activeForm,
                              currentHp: entry.wildShapes.activeForm.currentHp + 1,
                            },
                          }
                        : entry.wildShapes,
                    }))
            ...
            <div>
              ...
              <p>
                Retained mental stats: Int {formCompare.baseMental.intelligence}, Wis{' '}
                {formCompare.baseMental.wisdom}, Cha {formCompare.baseMental.charisma}
              </p>
            </div>
          </div>
        </SectionCard>
      ...
      <SectionCard title="Conditions and Notes">
        ...
        <textarea
          ...
          onChange={(event) => patch((entry) => ({ ...entry, notes: event.target.value }))}
        />
      </SectionCard>
    </div>
  );
