...
export const useCurrentCharacter = () => {
  ...
  const character = useMemo(() => characters.find((entry) => entry.id === characterId) ?? null, [characterId, characters]);
  const linkedCompanions = useMemo(() => companions.filter((entry) => entry.parentCharacterId === characterId), [characterId, companions]);
